"""
Umfassender OOS Backtest fuer M5 Swing.

Testet:
1. Monatliche Performance
2. Session-Analyse
3. Drawdown-Analyse
...
from config import MODELS_DIR, REPORTS_DIR, POINT
...
def main():
    print("=" * 70)
    print("UMFASSENDER OOS BACKTEST: M5 SWING")
    print("=" * 70)
    ...
    # Signale
    signals_long = generate_signals(test_df, model_long, feature_cols, 0.55, 10)
    signals_short = generate_signals(test_df, model_short, feature_cols, 0.55, 10)
    ...
    for name, stats in [("LONG", stats_long), ("SHORT", stats_short)]:
        if stats:
            ...
            print(f"    Total Profit: {stats['total_profit']:.0f} Punkte")
            print(f"    Max Drawdown: {stats['max_drawdown']:.0f} Punkte")
            print(f"    Sharpe: {stats['sharpe_ratio']:.1f}")
            print(f"    TP-Hit: {stats['tp_hit_rate']*100:.1f}%")
            print(f"    SL-Hit: {stats['sl_hit_rate']*100:.1f}%")
    ...
    if len(trades_short) > 0:
        ...
        monthly = trades_short.groupby("month").agg(
            n=("pnl", "count"), profit=("pnl", "sum"),
            win_rate=("pnl", lambda x: (x > 0).mean())
        ).reset_index()
    ...
    # === Session ===
    if len(trades_long) > 0:
        trades_long["hour"] = trades_long["entry_time"].dt.hour
        trades_long["session"] = "Other"
        trades_long.loc[(trades_long["hour"] >= 8) & (trades_long["hour"] < 13), "session"] = "London"
        trades_long.loc[(trades_long["hour"] >= 13) & (trades_long["hour"] < 16), "session"] = "LON/NY"
        trades_long.loc[(trades_long["hour"] >= 16) & (trades_long["hour"] < 20), "session"] = "NY"

        session = trades_long.groupby("session").agg(
            n=("pnl", "count"), profit=("pnl", "sum"),
            win_rate=("pnl", lambda x: (x > 0).mean())
        ).reset_index()
        ...
        for _, row in session.iterrows():
            print(f"    {row['session']:10s}: {row['n']:3d} Trades | Profit: {row['profit']:6.1f} | Win: {row['win_rate']*100:.0f}%")

    # === Statistische Signifikanz ===
    if len(trades_long) > 0 and len(trades_short) > 0:
        all_pnl = np.concatenate([trades_long["pnl"].values, trades_short["pnl"].values])

        # t-Test
        t_stat, p_value = scipy_stats.ttest_1samp(all_pnl, 0)
        p_one_tailed = p_value / 2 if t_stat > 0 else 1 - p_value / 2

        print(f"\n  STATISTISCHE SIGNIFIKANZ:")
        print(f"    t-Stat: {t_stat:.3f}")
        print(f"    p-Wert (one-tailed): {p_one_tailed:.6f}")
        print(f"    Signifikant (p<0.05): {'JA' if p_one_tailed < 0.05 else 'NEIN'}")

        # Sharpe
        sharpe = all_pnl.mean() / all_pnl.std() * np.sqrt(252 * 24 * 60 / 5)
