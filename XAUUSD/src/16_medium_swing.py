"""
Mittlerer Swing: Hohe Winrate, 5-10 Trades/Tag, hohe Genauigkeit.

Ziel:
- 5-10 Trades pro Tag
- Hohe Winrate (60%+)
- TP/SL: Mittelgroß (60-120 / 30-60)
- Horizont: 15-30 Minuten
- Balanciertes Target (35-50% positiv)
"""
...
def find_best_medium_swing_params(df):
    """Findet beste Parameter für mittleren Swing."""
    print("\n--- Mittleren Swing optimieren ---")

    close_arr = df["close"].values.astype(np.float64)
    high_arr = df["high"].values.astype(np.float64)
    low_arr = df["low"].values.astype(np.float64)

    # Mittlerer Swing: H=15-30, TP=60-120, SL=20-60
    configs = []
    for horizon in [10, 15, 20, 25, 30]:
        for tp in [50, 60, 70, 80, 90, 100, 120]:
            for sl in [20, 25, 30, 40, 50, 60]:
                if tp / sl < 1.5:
                    continue
                if tp / sl > 4:
                    continue
                ...
                # Balanciert: 30-50% positiv
                if 0.30 <= pos_rate <= 0.50:
                    configs.append({
                        "horizon": horizon,
                        "tp": tp,
                        "sl": sl,
                        "rr": tp / sl,
                        "pos_rate": pos_rate,
                        "score": 1 - abs(pos_rate - 0.40),  # 40% ist ideal
                    })

    configs_df = pd.DataFrame(configs).sort_values("score", ascending=False)

    print(f"\n  Top 15 Mittlere Swing-Konfigurationen:")
    print(f"  {'H':>4s} | {'TP':>5s} | {'SL':>5s} | {'R:R':>5s} | {'Pos%':>6s} | {'Score':>6s}")
    print(f"  {'-'*45}")
    for _, row in configs_df.head(15).iterrows():
        print(f"  {int(row['horizon']):4d} | {int(row['tp']):5d} | {int(row['sl']):5d} | "
              f"{row['rr']:5.1f} | {row['pos_rate']*100:5.1f}% | {row['score']:6.3f}")

    return configs_df


def train_medium_swing_model(df, feature_cols, horizon, tp, sl):
    """Trainiert ein Modell für mittleren Swing."""
    close_arr = df["close"].values.astype(np.float64)
    high_arr = df["high"].values.astype(np.float64)
    low_arr = df["low"].values.astype(np.float64)

    df["target_med_long"] = compute_target(close_arr, high_arr, low_arr, horizon, tp, sl, "long")
    df["target_med_short"] = compute_target(close_arr, high_arr, low_arr, horizon, tp, sl, "short")
    ...
    # LONG
    y_train = train_df["target_med_long"].values.astype(np.float32)
    y_val = val_df["target_med_long"].values.astype(np.float32)
    y_test = test_df["target_med_long"].values.astype(np.float32)
    ...
    test_auc_long = roc_auc_score(y_test, test_pred)

    # SHORT
    y_train_s = train_df["target_med_short"].values.astype(np.float32)
    y_val_s = val_df["target_med_short"].values.astype(np.float32)
    y_test_s = test_df["target_med_short"].values.astype(np.float32)
    ...
    test_auc_short = roc_auc_score(y_test_s, test_pred_s)

    return model_long, model_short, test_auc_long, test_auc_short


def generate_signals(df, model, feature_cols, conf_threshold=0.70, max_daily=10):
    """Generiert Signale mit Session-Filtern."""
...
def main():
    print("=" * 70)
    print("MITTLERER SWING: HOHE WINRATE, 5-10 TRADES/TAG")
    print("=" * 70)
    ...
    configs_df = find_best_medium_swing_params(df)

    if len(configs_df) == 0:
        print("  Keine passende Konfiguration gefunden!")
        return

    # Top-3 Konfigurationen trainieren
    results = []
    for idx, config in configs_df.head(3).iterrows():
        ...
        model_long, model_short, auc_long, auc_short = train_medium_swing_model(
            df.copy(), feature_cols, horizon, tp, sl
        )

        print(f"  LONG AUC: {auc_long:.4f}")
        print(f"  SHORT AUC: {auc_short:.4f}")
        print(f"  AVG AUC: {(auc_long + auc_short) / 2:.4f}")

        # Speichern
        long_path = os.path.join(MODELS_DIR, f"xgboost_medswing_h{horizon}_tp{tp}_sl{sl}.pkl")
        short_path = os.path.join(MODELS_DIR, f"xgboost_medswing_short_h{horizon}_tp{tp}_sl{sl}.pkl")
        ...
        if stats_long and stats_short:
            ...
            results.append({
                ...
                "long_trades": stats_long["n_trades"],
                "short_trades": stats_short["n_trades"],
                "total_trades": stats_long["n_trades"] + stats_short["n_trades"],
                "trades_per_day": (stats_long["n_trades"] + stats_short["n_trades"]) / n_days,
    ...
    print(f"\n\n{'='*70}")
    print("ERGEBNISSE: MITTLERER SWING")
    print(f"{'='*70}")
    print(f"  {'H':>4s} | {'TP':>4s} | {'SL':>4s} | {'Tr/D':>5s} | {'L-Win':>6s} | {'S-Win':>6s} | {'L-PF':>5s} | {'S-PF':>5s} | {'AvgAUC':>6s}")
    print(f"  {'-'*75}")
    for _, row in results_df.iterrows():
        print(f"  {int(row['horizon']):4d} | {int(row['tp']):4d} | {int(row['sl']):4d} | "
              f"{row['trades_per_day']:5.1f} | {row['long_winrate']*100:5.1f}% | "
              f"{row['short_winrate']*100:5.1f}% | {row['long_pf']:5.2f} | "
              f"{row['short_pf']:5.2f} | {row['avg_auc']:6.4f}")
